...
export class VotingsDetailsComponent implements OnInit {
  ...
  async updateVoting() {
    ...
    this.toaster.success('Voto actualizado!');
  }

  async deleteVoting() {
    await this.dataService.deleteVoting(this.voting.id);
    this.toaster.info('Voto eliminado!');
  ...
  copyUrlToClipboard() {
    const shareableUrl = `${window.location.origin}/app/voting/${this.voting.id}`;
    console.log(shareableUrl);

    this.clipboard.copy(shareableUrl);
    this.toaster.info('URL copiada al portapapeles');
  }
